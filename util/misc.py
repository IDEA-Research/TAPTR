...
def temporal_collate_fn(batch):
    ...
    batch = list(zip(*batch))
    batch[0] = nested_temporal_tensor_from_tensor_list(batch[0])
    return tuple(batch)
...
def nested_temporal_tensor_from_tensor_list(tensor_list: List[Tensor]):
    # TODO make this more general
    if tensor_list[0].ndim == 4:
        ...
        for b_id, (imgs, pad_imgs, ms) in enumerate(zip(tensor_list, tensor, mask)):
            for t_id, (img, pad_img, m) in enumerate(zip(imgs, pad_imgs, ms)):
                pad_img[: img.shape[0], : img.shape[1], : img.shape[2]].copy_(img)
                m[: img.shape[1], :img.shape[2]] = False
    else:
        raise ValueError('not supported')
