...
def get_args_parser():
    ...
    parser.add_argument('--support_points_mode', default='random', type=str,
                        help='random, grid, local or local_and_grid')
    parser.add_argument('--streaming', action='store_true',
                        help='Use streaming inference mode instead of offline mode')
...
def main(args):
    ...
    for data_id in tqdm(range(len(dataset_val))):
        if not ("strided" in args.dataset_file):  # first.
            ...
            with torch.no_grad():
                if args.streaming:
                    outputs, targets = model.streaming_forward(samples, [targets])
                else:
        ...
        if args.streaming:
            ...
            pred_occluded = outputs["pred_logits"].permute(1, 0, 2)[..., 1].sigmoid() > threshold_occ
        else:
