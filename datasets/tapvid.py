...
class TapVidDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.grid_size = 5
        self.support_points_mode = support_points_mode

    def padding_points(self, points_xy, points_occ, height=256, width=256, margin_x=0, margin_y=0, num_padding=-1):
        def generate_grid_coordinates(num_edge_points, lower_bound, upper_bound):        
            linspace = np.linspace(lower_bound, upper_bound, num_edge_points)
            grid_x, grid_y = np.meshgrid(linspace, linspace)
            grid_coordinates = np.stack([grid_x.flatten(), grid_y.flatten()], axis=-1)
            return grid_coordinates
        def generate_local_coordinates(points_xy, grid_size=6, exclude_center=True):
            num_points = points_xy.shape[0]
            # offset = np.linspace(-grid_size*3, grid_size*3, grid_size)
            offset = np.linspace(-24, 24, grid_size)
            grid_x, grid_y = np.meshgrid(offset, offset)
            grid = np.stack([grid_x.flatten(), grid_y.flatten()], axis=-1)  # (grid_size*grid_size, 2)
            if exclude_center:
                center_index = grid_size * grid_size // 2
                grid = np.delete(grid, center_index, axis=0)
            expanded_points = points_xy[:, None, :]
            grid = grid[None,:,:]
            grid = np.repeat(grid, num_points, axis=0)
            local_coordinates = expanded_points + grid
            local_coordinates = local_coordinates.reshape(-1, 2)
            return local_coordinates
        
        if self.support_points_mode == "random":
        ...
        elif self.support_points_mode == "grid":
            margin = width // 16
            num_edge_points = 5
            grid_coordinates = generate_grid_coordinates(num_edge_points, margin, width - 1 - margin)   # num_edge_points*num_edge_points, 2
            grid_coordinates = grid_coordinates[:,None].repeat(points_xy.shape[1], axis=1)  # num_edge_points*num_edge_points, num_frames, 2
            points_xy = np.concatenate((points_xy, grid_coordinates), axis=0)
            # occ
            padding_grid_points_occ = np.ones((num_edge_points*num_edge_points, points_occ.shape[1])) < 0
            padding_grid_points_occ[:, :12] = \
                padding_grid_points_occ[:, :12] | \
                np.arange(12)[None,:].repeat(padding_grid_points_occ.shape[0], 0) < np.random.randint(0, 12, padding_grid_points_occ.shape[0])[:,None]
            points_occ = np.concatenate((points_occ, padding_grid_points_occ), axis=0)

        elif self.support_points_mode == "local":
            first_emerge_frame = np.argmax(~points_occ, axis=1)
            never_emerge_rows = np.all(points_occ, axis=1)
            first_emerge_frame[never_emerge_rows] = points_occ.shape[1] - 1

            num_origin_points = points_xy.shape[0]
            local_grid_size = 8
            if local_grid_size % 2 == 0:
                exclude_center = False
            else:
                exclude_center = True
            first_emerge_position = points_xy[np.arange(num_origin_points), first_emerge_frame]
            local_coordinates = generate_local_coordinates(first_emerge_position, local_grid_size, exclude_center)
            local_coordinates = local_coordinates[:,None].repeat(points_xy.shape[1], axis=1)
            points_xy = np.concatenate((points_xy, local_coordinates), axis=0)
            # occ
            if exclude_center:
                padding_local_points_occ = np.repeat(points_occ, local_grid_size*local_grid_size-1, axis=0)
            else:
                padding_local_points_occ = np.repeat(points_occ, local_grid_size*local_grid_size, axis=0)
            points_occ = np.concatenate((points_occ, padding_local_points_occ), axis=0)

        elif self.support_points_mode == "local_and_grid":
            first_emerge_frame = np.argmax(~points_occ, axis=1)
            never_emerge_rows = np.all(points_occ, axis=1)
            first_emerge_frame[never_emerge_rows] = points_occ.shape[1] - 1
            ...
            margin = width // 16
            num_edge_points = 6
            grid_coordinates = generate_grid_coordinates(num_edge_points, margin, width - 1 - margin)
            grid_coordinates = grid_coordinates[:,None].repeat(points_xy.shape[1], axis=1)

            points_xy = np.concatenate((points_xy, local_coordinates, grid_coordinates), axis=0)
            ...
            points_occ = np.concatenate((points_occ, padding_local_points_occ, padding_grid_points_occ), axis=0)
        
        elif self.support_points_mode == "none":
            pass
        else:
...
def build_tapvid(image_set, args):
    ...
    PATHS = {
        ...
        "tapvid_robotap_first": ("robotap", root / "tapvid"/ "robotap", True, True),
    }
    ...
    dataset = TapVidDataset(
        ...
        support_points_mode=args.support_points_mode,
    )
    return dataset
