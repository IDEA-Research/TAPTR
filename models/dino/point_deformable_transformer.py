...
class PointDeformableTransformer(DeformableTransformer):
    ...
    def __init__(self, 
        ...
        self.use_cca_in_apu = use_cca_in_apu
    ...
    def forward_pt_tracking_decoder(self, 
            ...
            tgt_long_temporal_feature, long_temp_tgt_mask,
        ...
        outputs = self.decoder(
                ...
                tgt_long_temporal_feature=tgt_long_temporal_feature if tgt_long_temporal_feature is None else tgt_long_temporal_feature.transpose(0, 1),
                long_temp_tgt_mask=long_temp_tgt_mask,
    ...
    def prepare_point_query(self, flattened_multi_scale_features, flattened_multi_scale_pos_enc, flattened_multi_scale_mask, level_start_index, multi_scale_spatial_shapes, pt_refpoint, pt_tgt, first_emerge_frame=None):
        ...
        Args:
            ...
            pt_refpoint (torch.tensor): [bs, len_temp, num_point, 2], the points' initial positions.
        ...
        def generate_square_coordinates(coords, length, n):
            """
            generates coordinates within a square based on each coordinate.

            inputs:
                coords: [bs, num_point, 2]
                length: float, the length of the square
                n: int, the number of points generated in each direction, (2n+1)^2 points in total
            outputs:
                square_coords: [bs, num_point, (2n+1)^2, 2], the generated coordinates
            """
            offset = torch.linspace(-length / 2, length / 2, steps=2*n + 1, device=coords.device)
            grid_x, grid_y = torch.meshgrid(offset, offset)
            grid = torch.stack([grid_x, grid_y], dim=-1).view(-1, 2)  # [(2n+1)*(2n+1), 2]
            square_coords = coords.unsqueeze(-2) + grid.view(1, 1, -1, 2)    # [bs, num_point, (2n+1)^2, 2]
            return square_coords
        
        bs, len_temp, num_points = pt_refpoint.shape[:3]
        pt_features = []
        pt_context_features = []
        if self.use_cca_in_apu is not None:
            length = self.use_cca_in_apu[0]
            n = self.use_cca_in_apu[1]
            for init_feature_mode in self.query_feature_initialize_mode:
                if init_feature_mode == "first_emerge_frame_ms_feature_bilinear":
                    for spatial_shape, start_index in zip(multi_scale_spatial_shapes, level_start_index):
                        feature_maps = flattened_multi_scale_features[:, start_index: start_index+spatial_shape.prod()].view(bs, len_temp, *spatial_shape, -1).permute(0,1,4,2,3)  # bs len_temp c h w
                        context_points = generate_square_coordinates(pt_refpoint[:,0] * (torch.flip(spatial_shape, dims=[0])-1), length, n)  # [bs, num_point, (2n+1)^2, 2]
                        context_points  = context_points.view(bs, -1, 2) # [bs, num_points*(2n+1)^2, 2]
                        first_emerge_frame_repeat = first_emerge_frame[..., None].repeat_interleave((2*n+1)**2, dim=1)  # [bs, num_points*(2n+1)^2, 1]
                        sampling_loc_3d = torch.cat([first_emerge_frame_repeat, context_points], dim=-1)[:,None]   # [bs, 1, num_points*(2n+1)^2, 3]
                        pt_local_feature_lvl = sample_features5d(feature_maps, sampling_loc_3d)  # [bs, 1, num_points*(2n+1)^2, C]
                        pt_local_feature_lvl = pt_local_feature_lvl.squeeze(1).view(bs, num_points, 2*n+1, 2*n+1, -1)   # [bs, num_points, 2n+1, 2n+1, C]
                        pt_context_features.append(pt_local_feature_lvl)   # [bs, num_points, 2n+1, 2n+1, C]
                        pt_features.append(pt_local_feature_lvl[:,:,n,n,:])     # [bs, num_points, C]
                else:
            ...
            for init_feature_mode in self.query_feature_initialize_mode:
                if init_feature_mode == "first_emerge_frame_ms_feature_bilinear":
                    for spatial_shape, start_index in zip(multi_scale_spatial_shapes, level_start_index):
                        feature_maps = flattened_multi_scale_features[:, start_index: start_index+spatial_shape.prod()].view(bs, len_temp, *spatial_shape, -1).permute(0,1,4,2,3)  # bs len_temp c h w
                        sampling_loc_3d = torch.cat([first_emerge_frame[..., None], pt_refpoint[:,0] * (spatial_shape-1)], dim=-1)[:,None]  # bs 1 num_point 3
                        pt_local_feature_lvl = sample_features5d(feature_maps, sampling_loc_3d)  # bs 1 num_point C
                        pt_features.append(pt_local_feature_lvl.squeeze(1))     # bs num_point C
                else:
                    raise NotImplementedError
        # summarize the point features.
        point_feature = self.query_feature_initializer(torch.cat(pt_features, dim=2))
        if len(pt_context_features) > 0:
            point_context_features = self.query_feature_initializer(torch.cat(pt_context_features, dim=-1))  # [bs, num_points, 2n+1, 2n+1, C]
        else:
            point_context_features = None
        ...
        point_traj_visib = inverse_sigmoid(point_traj_visib)
        return point_traj_feature, point_traj_embed, point_traj_visib, point_context_features


    def streaming_prepare_point_query(self, 
            frame_id, flattened_multi_scale_features, 
            level_start_index, multi_scale_spatial_shapes, 
            pt_refpoint, pt_tgt, first_emerge_frame,
            point_traj_features, point_context_features, point_traj_embeds, point_traj_embeds_ca, point_traj_visibility
        ):
        """prepare the initial feature of each points according to the "query_feature_initialize_mode".
            These features are supported: first_frame_feature_bilinear, first_frame_pe_bilinear, initial_pe, initial_visib.
            Please refer to the docstring of the class for more detail.
        ...
        Args:
            ...
            flattened_multi_scale_features (torch.tensor): [bs*1, \sum(hw), C], the flattened multi_scale feature maps of current frame.
            level_start_index (torch.tensor): [num_scales], the start index of multi_scale feature maps in the flattend version.
            multi_scale_spatial_shapes (torch.tensor): [num_scales, 2], the resolution of multi_scale feature maps.
            pt_refpoint (torch.tensor): [bs, len_temp, num_point, 2], the points' initial positions.
            pt_tgt (torch.tensor): [bs, len_temp, num_points], the points' initial visibility.
            first_emerge_frame: [bs, num_point]

            point_traj_features (list[torch.tensor]) len = num_dec_layers: [bs*len_temp, num_point, d_model]
            point_traj_embeds  (list[torch.tensor]) len = num_dec_layers: [bs*len_temp, num_point, 4]
            point_traj_visibility (list[torch.tensor]) len = num_dec_layers: [bs*len_temp, num_point, 3]
            point_context_features (list[torch.tensor]) len = num_dec_layers: [bs, num_points, 2n+1, 2n+1, C]
        ...
        if (first_emerge_frame == frame_id).sum() == 0:  # if no points emerge in the current frame, we do not prepare any point query, no need to update the states of the points.
            return point_traj_features, point_context_features, point_traj_embeds, point_traj_embeds_ca, point_traj_visibility
        
        points_to_prepare = (first_emerge_frame == frame_id)  # [bs, num_points]
        bs, len_temp, num_points = pt_refpoint.shape[:3]
        assert bs == 1, "Currently, we only support batch size 1 in streaming forward."
        num_points_to_prepare = points_to_prepare.sum(dim=-1)[0]
        ...
        if self.use_cca_in_apu is not None:
            ...
            for init_feature_mode in self.query_feature_initialize_mode:
                if init_feature_mode == "first_emerge_frame_ms_feature_bilinear":
                    for spatial_shape, start_index in zip(multi_scale_spatial_shapes, level_start_index):
                        feature_maps = flattened_multi_scale_features[:, start_index: start_index+spatial_shape.prod()].view(bs, 1, *spatial_shape, -1).permute(0,1,4,2,3)  # bs 1 c h w
                        context_points = generate_square_coordinates(pt_refpoint[:,0, points_to_prepare.nonzero()[:, 1]] * (torch.flip(spatial_shape, dims=[0])-1), length, n)  # [bs, num_point_to_prepare, (2n+1)^2, 2]
                        context_points  = context_points.view(bs, -1, 2) # [bs, num_points*(2n+1)^2, 2]
                        # first_emerge_frame_repeat = first_emerge_frame[..., None].repeat_interleave((2*n+1)**2, dim=1)  # [bs, num_points_to_prepare*(2n+1)^2, 1]
                        first_emerge_frame_repeat = torch.ones([bs, num_points_to_prepare * (2 * n + 1)**2, 1], device=pt_refpoint.device) * frame_id  #* this operation limites streaming_prepare_point_query can only process when bs == 1.
                        sampling_loc_3d = torch.cat([first_emerge_frame_repeat, context_points], dim=-1)[:,None]   # [bs, 1, num_points*(2n+1)^2, 3]
                        pt_local_feature_lvl = sample_features5d(feature_maps, sampling_loc_3d)  # [bs, 1, num_points*(2n+1)^2, C]
                        pt_local_feature_lvl = pt_local_feature_lvl.squeeze(1).view(bs, num_points_to_prepare, 2*n+1, 2*n+1, -1)   # [bs, num_points, 2n+1, 2n+1, C]
                        pt_context_features.append(pt_local_feature_lvl)   # [bs, num_points, 2n+1, 2n+1, C]
                        pt_features.append(pt_local_feature_lvl[:,:,n,n,:])     # [bs, num_points, C]
                else:
                    raise NotImplementedError
            ...
            raise NotImplementedError
        # summarize the point features.
        point_feature = self.query_feature_initializer(torch.cat(pt_features, dim=2))   # fuse the features
        if len(pt_context_features) > 0:
            point_context_feature_ = self.query_feature_initializer(torch.cat(pt_context_features, dim=-1))  # [bs, num_points, 2n+1, 2n+1, C]
        else:
            point_context_feature_ = None
        point_embed   = torch.cat([
            inverse_sigmoid(pt_refpoint[:,0, points_to_prepare.nonzero()[:, 1]]),
            inverse_sigmoid(torch.ones_like(pt_refpoint[:,0, points_to_prepare.nonzero()[:, 1]]) * 0.5)
        ], dim=2)
        point_visibility = inverse_sigmoid(F.one_hot(pt_tgt[..., points_to_prepare.nonzero()[:, 1]], num_classes=3).float())

        # update the initial states for the points that start tracking from the current frame.
        if point_traj_features.dtype != point_feature.dtype:
            # if we use fp16, the prepared placeholder will have different dtype with the point_feature, we need to convert the dtype.
            point_traj_features = point_traj_features.to(point_feature.dtype)
            point_context_features = point_context_features.to(point_context_feature_.dtype)
        point_traj_features[:, points_to_prepare.nonzero()[:, 1]] = point_feature.squeeze(0)   # [bs*len_temp, num_point, d_model]
        point_context_features[:, points_to_prepare.nonzero()[:, 1]] = point_context_feature_  # [bs, num_point, 2n+1, 2n+1, C]
        for layer_id in range(len(point_traj_embeds)):
            point_traj_embeds[layer_id][:, points_to_prepare.nonzero()[:, 1]] = point_embed.squeeze(0)   # [bs*len_temp, num_point, 4]
            point_traj_embeds_ca[layer_id][:, points_to_prepare.nonzero()[:, 1]] = point_embed.squeeze(0)   # [bs*len_temp, num_point, 4]
            point_traj_visibility[layer_id][:, points_to_prepare.nonzero()[:, 1]] = point_visibility.squeeze(0)  # bs, len_temporal, num_points 3
        return point_traj_features, point_context_features, point_traj_embeds, point_traj_embeds_ca, point_traj_visibility
...
def build_point_deformable_transformer(args):
    ...
    use_cca_in_apu = getattr(args, "use_cca_in_apu", None)
