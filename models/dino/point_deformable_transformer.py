...
from .ops.modules import MSDeformAttn, MSDeformAttn_NotLearnable
...
class PointDeformableTransformer(DeformableTransformer):
    ...
    """
    def __init__(self, 
            d_model=256, nhead=8, num_queries=300, num_encoder_layers=6, num_unicoder_layers=0, num_decoder_layers=6, dim_feedforward=2048, dropout=0, activation="relu", normalize_before=False, return_intermediate_dec=False, query_dim=4, num_patterns=0, modulate_hw_attn=False, deformable_encoder=False, deformable_decoder=False, num_feature_levels=1, enc_n_points=4, dec_n_points=4, use_deformable_box_attn=False, box_attn_type='roi_align', learnable_tgt_init=False, decoder_query_perturber=None, add_channel_attention=False, add_pos_value=False, random_refpoints_xy=False, two_stage_type='no', two_stage_pat_embed=0, two_stage_add_query_num=0, two_stage_learn_wh=False, two_stage_keep_all_tokens=False, dec_layer_number=None, rm_enc_query_scale=True, rm_dec_query_scale=True, rm_self_attn_layers=None, key_aware_type=None, layer_share_type=None, rm_detach=None, decoder_sa_type='ca', module_seq=['sa', 'ca', 'ffn'], embed_init_tgt=False, use_detached_boxes_dec_out=False,
            ...
            num_global_decoder_layers=0, global_corr_once=False,
            fix_decoder_unsync=False,
            ignore_wh_inca=False,
            position_aware_mem_mode=False,
            use_checkpoint=False,
        ):
        super().__init__(d_model, nhead, num_queries, num_encoder_layers, num_unicoder_layers, num_decoder_layers, dim_feedforward, dropout, activation, normalize_before, return_intermediate_dec, query_dim, num_patterns, modulate_hw_attn, deformable_encoder, deformable_decoder, num_feature_levels, enc_n_points, dec_n_points, use_deformable_box_attn, box_attn_type, learnable_tgt_init, decoder_query_perturber, add_channel_attention, add_pos_value, random_refpoints_xy, two_stage_type, two_stage_pat_embed, two_stage_add_query_num, two_stage_learn_wh, two_stage_keep_all_tokens, dec_layer_number, rm_enc_query_scale, rm_dec_query_scale, rm_self_attn_layers, key_aware_type, layer_share_type, rm_detach, decoder_sa_type, module_seq, embed_init_tgt, use_detached_boxes_dec_out, num_global_decoder_layers, global_corr_once, fix_decoder_unsync, ignore_wh_inca, position_aware_mem_mode, use_checkpoint)
        ...
        self.point_size_initialize_value = None
    ...
    def prepare_point_query(self, flattened_multi_scale_features, flattened_multi_scale_pos_enc, flattened_multi_scale_mask, level_start_index, multi_scale_spatial_shapes, pt_refpoint, pt_tgt, first_emerge_frame=None):
        ...
        point_embed   = torch.cat([
            ...
            inverse_sigmoid(torch.ones_like(pt_refpoint[:, 0]) * self.point_size_initialize_value)
        ], dim=2)
...
def build_point_deformable_transformer(args):
    ...
    num_global_decoder_layers = getattr(args, "num_global_decoder_layers", 0)
    global_corr_once = getattr(args, "global_corr_once", False)
    fix_decoder_unsync = getattr(args, "fix_decoder_unsync", False)
    ignore_wh_inca = getattr(args, "ignore_wh_inca", False)
    position_aware_mem_mode = getattr(args, "position_aware_mem_mode", "no")
    ...
    return PointDeformableTransformer(
        ...
        num_global_decoder_layers=num_global_decoder_layers,
