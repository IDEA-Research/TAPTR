...
class TAPTR(DINO):
    def __init__(self, 
            ...
            use_lta_and_no_content_padding=False,
            visibility_aware_in_lta=False,
            streaming_limited_size_of_placeholder=-1,
        ):
        ...
        self.use_lta_and_no_content_padding = use_lta_and_no_content_padding
        if not self.use_lta_and_no_content_padding:
        ...
        self.dropout_window_update = nn.Dropout(0.6)
        self.visibility_aware_in_lta = visibility_aware_in_lta

        # for streaming inference
        self.streaming_limited_size_of_placeholder = streaming_limited_size_of_placeholder
    ...
    def prepare_point_initial_states(self, targets, batch_size, len_temporal):
        ...
        if self.visibility_aware_in_lta:
            pt_initial_visibility = pt_initial_location.new_ones(pt_initial_location.shape[:3]).long() * 2  # initialized as unvisible for each point in every frame
            pt_initial_visibility = torch.masked_fill(pt_initial_visibility, torch.logical_xor(pt_updating_masks, pt_tracking_masks).permute(0,2,1), 1.0)  # initialize as visible for each point's emerging frame.
        else:
    ...
    def prepare_window_forward(self, 
                              ...
                              pt_current_traj_features, pt_current_traj_embeds, pt_current_traj_visibs,
        ...
        window_long_temp_track_mask = pt_tracking_masks[:,:,:window_tail+1].clone()
        window_long_temp_track_visib_weight = pt_current_traj_visibs[:, :window_tail+1].permute(0, 2, 1).sigmoid().detach()
        ...
        # Long Temporal Attention Mask.
        if self.visibility_aware_in_lta:
            emerge_mask = torch.logical_xor(window_long_temp_track_mask, pt_updating_masks[:,:,:window_tail+1])
            window_long_temp_track_visib_weight[emerge_mask] = 1.0
            flag_weight_0 = (window_track_mask[..., None].int() + window_long_temp_track_mask[..., None, :].int()) == 1
            window_ltemp_attn_mask = window_track_mask[..., None].float()*0.0 + window_long_temp_track_mask[..., None, :].float() * window_long_temp_track_visib_weight[..., None, :]
            window_ltemp_attn_mask[flag_weight_0] = -torch.inf
            window_ltemp_attn_mask = window_ltemp_attn_mask[:, :, None].repeat(1, 1, n_head_temp_self_attn, 1, 1).flatten(0,2) 
            # window_ltemp_attn_mask will be multiplied to the attention scores before softmax.
        else:
            window_ltemp_attn_mask = (window_track_mask[..., None].int() + window_long_temp_track_mask[..., None, :].int()) == 1  # bs num_points, window_size, len_long_temp
            window_ltemp_attn_mask = window_ltemp_attn_mask[:, :, None].repeat(1, 1, n_head_temp_self_attn, 1, 1).flatten(0,2)  # bs, num_point, num_head, window_size, len_long_temp --> bs*num_point*num_head, window_size, len_long_temp
        ...
        return window_memory, window_lvl_pos_embed_flatten, window_mask_flatten, window_valid_ratios, \
            window_temp_attn_mask, window_ltemp_attn_mask, window_self_attn_mask, window_track_mask, window_update_mask, \
            pt_window_traj_feature, pt_window_traj_embeds, pt_window_traj_corr_feature

    def update_query_states(self, window_head, window_tail, window_update_mask, pt_current_traj_embeds, pt_current_traj_visibility, pt_current_traj_features, pt_initial_traj_features, outputs_coord_list, outputs_class, outputs_hs, pt_current_traj_embeds_ca, outputs_coord_list_ca, padding_to_future=True):
        ...
        len_temporal = pt_current_traj_embeds[0].shape[1]
        for l_id in range(len(self.transformer.decoder.layers)):
            ...
            if padding_to_future:
                pt_current_traj_embeds[l_id][:, window_tail+1:] = pt_current_traj_embeds[l_id][:, window_tail].unsqueeze(1)
                #* do not padding the visibility score, because the score will be used in the next window when visibility-aware is True.
                if not self.visibility_aware_in_lta:
                    pt_current_traj_visibility[l_id][:, window_tail+1:] = pt_current_traj_visibility[l_id][:, window_tail].unsqueeze(1)
        if not (outputs_coord_list_ca is None):
            for l_id in range(len(self.transformer.decoder.layers)):
                pt_current_traj_embeds_ca[l_id][:, window_head:window_tail+1]     = pt_current_traj_embeds_ca[l_id].clone()[:, window_head:window_tail+1]     * (1 - window_update_mask_) + outputs_coord_list_ca[l_id].view(-1, self.sliding_window_size, *outputs_coord_list_ca.shape[2:]) * window_update_mask_
                if padding_to_future:
                    pt_current_traj_embeds_ca[l_id][:, window_tail+1:] = pt_current_traj_embeds_ca[l_id][:, window_tail].unsqueeze(1)
        
        # 2. updating of content feature.
        if self.use_lta_and_no_content_padding:
            pt_current_traj_features[:, window_head:window_head+self.sliding_window_stride] = outputs_hs[-1].view(-1, self.sliding_window_size, *outputs_hs[-1].shape[1:])[:, :self.sliding_window_stride]
        else:
            ...
            if not self.training and self.long_video_mode:
        ...
        return pt_current_traj_embeds, pt_current_traj_visibility, pt_current_traj_features, pt_current_traj_embeds_ca
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        ptq_current_traj_features, ptq_current_traj_embeds, ptq_current_traj_visibility, ptq_initial_context_features = self.transformer.prepare_point_query(
        ...
        ptq_current_traj_embeds = [ptq_current_traj_embeds.clone() for _ in range(len(self.transformer.decoder.layers))]
        ptq_current_traj_visibility = [ptq_current_traj_visibility.clone() for _ in range(len(self.transformer.decoder.layers))]
        ptq_current_traj_embeds_ca = deepcopy(ptq_current_traj_embeds)
        ...
        if ptq_initial_context_features is not None:
            ptq_initial_context_features = ptq_initial_context_features.repeat(self.sliding_window_size, 1, 1, 1, 1)
        for window_id in window_ids:
            self.window_id = window_id
            ptq_visibility_weights = [aa[..., 1] for aa in ptq_current_traj_visibility]
            ...
            ptq_window_feature, ptq_window_embed, ptq_window_traj_corr_feature = self.prepare_window_forward(
                ...
                ptq_current_traj_embeds[-1],  # use the latest point states as the initial states of the window.
                ptq_visibility_weights[-1],
                ptq_initial_traj_features,
                ptq_group_mask,
            )
            if window_memory is None:
                # print("one window is skipped.")
                continue  # no points should be updated in the window.
            # 4. Forward each window.
            if window_id > 0:
                ptq_long_temporal_feature = ptq_current_traj_features.clone()[:, :window_tail+1].flatten(0,1)
            else:
                ptq_long_temporal_feature = None
            hs, reference, _, _, reference_ca = self.transformer.forward_pt_tracking_decoder(
                ...
                ptq_long_temporal_feature, window_ltemp_attn_mask, 
                ptq_initial_context_features,
            )
            # 5. Get decoder output.
            outputs_coord_list = []
            layer_refs = reference_ca
            for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_hs) in enumerate(zip(layer_refs, self.bbox_embed, hs)):
            ...
            if not (reference_ca is None):
                reference_ca = torch.stack(reference_ca)
                outputs_coord_list_ca = inverse_sigmoid(reference_ca)
            else:
                outputs_coord_list_ca = None
            ptq_current_traj_embeds, ptq_current_traj_visibility, ptq_current_traj_features, ptq_current_traj_embeds_ca = self.update_query_states(window_head, window_tail, window_update_mask, ptq_current_traj_embeds, ptq_current_traj_visibility, ptq_current_traj_features, ptq_initial_traj_features, outputs_coord_list, outputs_class, hs, ptq_current_traj_embeds_ca, outputs_coord_list_ca, padding_to_future=window_id<(num_windows-1))
        ...
        full_seq_output_ca = full_seq_targets_ca = None
        if self.training:
            ...
            ptq_current_traj_embeds_ca = torch.stack(ptq_current_traj_embeds_ca, dim=0).flatten(1,2)
            ptq_current_traj_embeds_ca = ptq_current_traj_embeds_ca.sigmoid()
            full_seq_output_ca, full_seq_targets_ca = self.prepare_window_loss(ptq_current_traj_visibility, ptq_current_traj_embeds_ca, targets, 0, len_temporal-1, ptq_updating_masks)
        else:
        ...
        outputs = {
            ...
            "full_seq_output_ca": full_seq_output_ca,
            "window_output_list": window_output_list,
        }
        targets = {
            "full_seq_target": full_seq_targets,
            "full_seq_target_ca": full_seq_targets_ca,
    ...
    def streaming_forward(self, samples: NestedTensor, targets:List=None, epoch :int=None):

        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_temporal_tensor_from_tensor_list(samples)
        if isinstance(samples, torch.Tensor):
            batch_size, len_temporal = samples.shape[:2]
        elif isinstance(samples, NestedTensor):
            batch_size, len_temporal = samples.tensors.shape[:2]
        #! 1. Prepare the initial states the point queries (ptq). Use the states when their belonging points first emerge to initialize their states.
        ptq_tracking_masks, ptq_start_tracking_frames, ptq_updating_masks, \
        ptq_initial_traj_location, ptq_initial_traj_visibility, ptq_group_mask = self.prepare_point_initial_states(targets, batch_size, len_temporal)

        # Initialize the placehold of all the required outputs.
        num_layers = len(self.transformer.decoder.layers)
        num_points = targets[0]["pt_tracking_mask"].shape[0]
        num_scales = len(self.backbone[0].body.return_layers)
        ...
        ptq_current_traj_features, ptq_initial_context_features, \
        ptq_current_traj_embeds, ptq_current_traj_embeds_ca, \
        ptq_current_traj_visibility = \
            self.streaming_get_output_placeholder(batch_size, len_temporal, num_points, d_model, num_layers, device)

        # Streaming process.
        for frame_id in tqdm(range(len_temporal)):
            window_head, window_tail = self.get_window_range(frame_id, len_temporal)
            #! 2. Prepare frame feature.
            memory, lvl_pos_embed_flatten, level_start_index, spatial_shapes, mask_flatten, valid_ratios = self.streaming_prepare_video_features(frame_id, samples, device)
            #! 3. Prepare point query features. (not all of the points start to be tracked at the first frame, we should update the initial feature.)
            ptq_current_traj_features, ptq_initial_context_features, \
            ptq_current_traj_embeds, ptq_current_traj_embeds_ca, \
            ptq_current_traj_visibility = self.transformer.streaming_prepare_point_query(
                frame_id, memory, 
            ...
            # If none of the points should be tracked in the current frame, we skip it.
            if ptq_updating_masks[:,:,window_head:window_tail+1].sum() == 0:  
                continue
            #! 4. Prepare variables that are needed for decoder.
            self.window_id = frame_id
            ptq_visibility_weights = [aa[..., 1] for aa in ptq_current_traj_visibility]
            # window_memory, window_lvl_pos_embed_flatten, window_mask_flatten, window_valid_ratio, \
            window_temp_attn_mask, window_ltemp_attn_mask, window_self_attn_mask, window_track_mask, window_update_mask, \
            ptq_window_feature, ptq_window_embed, ptq_window_traj_corr_feature = self.streaming_prepare_window_forward(
                batch_size, len_temporal, window_head, window_tail,
                ptq_tracking_masks, ptq_updating_masks, 
                ptq_current_traj_features, 
                ptq_current_traj_embeds[-1],  # use the latest point states as the initial states of the window.
                ptq_visibility_weights[-1], 
                ptq_group_mask,
            )
            # TODO: add global matching.
            if frame_id > 0:
                ptq_long_temporal_feature = ptq_current_traj_features.clone()[:window_tail+1]
                if self.streaming_limited_size_of_placeholder != -1:
                    window_ltemp_attn_mask = window_ltemp_attn_mask[..., -self.streaming_limited_size_of_placeholder:]
            else:
                ptq_long_temporal_feature = None
            #! 5. Get decoder's multi-layer refinement output.
            hs, reference, _, _, reference_ca = self.transformer.forward_pt_tracking_decoder(
                memory, lvl_pos_embed_flatten, level_start_index, spatial_shapes, 
                mask_flatten, valid_ratios,
                ptq_window_feature, ptq_window_embed, ptq_window_traj_corr_feature,
                self.sliding_window_size, window_temp_attn_mask, window_self_attn_mask, window_update_mask,
                ptq_long_temporal_feature, window_ltemp_attn_mask, ptq_initial_context_features,
            )
            #! 6. Update the point query states with decoder outputs.
            outputs_coord_list = []
            # if not self.fix_sync_ca_update_bug:
            #     layer_refs = reference[:-1]
            # else:
            layer_refs = reference_ca
            for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_hs) in enumerate(zip(layer_refs, self.bbox_embed, hs)):
                layer_delta_unsig = layer_bbox_embed(layer_hs)
            ...
            if not (reference_ca is None):
                ...
                outputs_coord_list_ca = None
            #* use global matching results to help scene switching cases.
            # if (not self.training) and self.use_globalmatching_when_switching:
            #     outputs_coord_list = self.replace_with_globalmatching_when_switching(targets[0]["scene_switching"], window_head, outputs_coord_list, global_matching_positions)
            global_matching_positions = None
            ptq_current_traj_features, \
            ptq_current_traj_embeds, ptq_current_traj_embeds_ca, \
            ptq_current_traj_visibility = self.streaming_update_query_states(
                window_head, window_tail, len_temporal, \
                window_update_mask, \
                ptq_current_traj_embeds, ptq_current_traj_embeds_ca, \
                ptq_current_traj_visibility, \
                ptq_current_traj_features, \
                outputs_coord_list, outputs_coord_list_ca, outputs_class, hs,\
                padding_to_future=frame_id<(len_temporal-1), \
                global_matching_positions=global_matching_positions
            )
            
            # transform query states into the readable format.
            outputs_coord_list = outputs_coord_list.sigmoid()
        #! 7. After sequential tracking, prepare for the final output.
        full_seq_output_ca = full_seq_targets_ca = None
        if self.training:
            ptq_current_traj_embeds = torch.stack(ptq_current_traj_embeds, dim=0).flatten(1,2)  # num_dec, bs, len_temporal, num_queries, 4 --> num_dec, bs*len_temporal, num_queries, 4
            ptq_current_traj_visibility = torch.stack(ptq_current_traj_visibility, dim=0).flatten(1,2)
            ...
            ptq_current_traj_embeds = ptq_current_traj_embeds[-1]  # num_dec, bs*len_temporal, num_queries, 4 --> bs*len_temporal, num_queries, 4
            ptq_current_traj_visibility = ptq_current_traj_visibility[-1]  # num_dec, bs*len_temporal, num_queries, 4 --> bs*len_temporal, num_queries, 3
            ptq_current_traj_embeds = ptq_current_traj_embeds.sigmoid()
            full_seq_output = {'pred_logits': ptq_current_traj_visibility, 'pred_boxes': ptq_current_traj_embeds, "dn_meta": None}
            # if global_matching_positions is not None:
            #     full_seq_output["pred_global"] = global_matching_positions
            #     full_seq_output["heat_map"] = heat_map  # [num_points, num_frames, h, w]
            full_seq_targets = []
            for target, ptq_update_mask in zip(targets, ptq_updating_masks):
                target["ptq_update_mask"] = ptq_update_mask
                full_seq_targets.append(target)
    ...
    def streaming_prepare_video_features(self, frame_id, video_data, device):
        """ Prepare feature maps for one single frame.
        Inputs:
            video_data: NestedTensor, [bs, len_temporal, 3, H, W]
            frame_id: int

        Outputs:
            memory: bs*1 \sigma_wh C
            lvl_pos_embed_flatten: bs*1 \sigma_wh C
            level_start_index: num_levels
            spatial_shapes: num_levels 2
            mask_flatten: bs*1 \sigma_wh
        """
        frame_data = nested_temporal_tensor_from_tensor_list(video_data.tensors[:, frame_id:frame_id+1].to(device))
        frame_data, batch_size, _ = self.flatten_temporal_dimension(frame_data)
        features, poss = self.backbone(frame_data)
        srcs = []
        masks = []
        for l, feat in enumerate(features):
            src, mask = feat.decompose()
    ...
    def streaming_get_output_placeholder(self, bs, len_temp, num_points, d_model, num_layers, device):
        if self.streaming_limited_size_of_placeholder == -1:
            placeholder_point_traj_feature = torch.ones([bs * len_temp, num_points, d_model], device=device)
        else:
            placeholder_point_traj_feature = torch.ones([bs * self.streaming_limited_size_of_placeholder, num_points, d_model], device=device)
        placeholder_point_context_feature = torch.ones([bs, num_points, (2 * self.transformer.use_cca_in_apu[1] + 1), (2 * self.transformer.use_cca_in_apu[1] + 1), d_model], device=device)
        placeholder_point_traj_embed   = torch.zeros([bs * len_temp, num_points, 4], device=device)
        placeholder_point_traj_visib = inverse_sigmoid(torch.cat([torch.zeros([bs * len_temp, num_points, 2], device=device), torch.ones([bs * len_temp, num_points, 1], device=device)], dim=-1))

        placeholder_point_traj_embed = [placeholder_point_traj_embed.clone() for _ in range(num_layers)]
        placeholder_point_traj_visib = [placeholder_point_traj_visib.clone() for _ in range(num_layers)]
        placeholder_point_traj_embed_ca = deepcopy(placeholder_point_traj_embed)
        return placeholder_point_traj_feature, placeholder_point_context_feature, placeholder_point_traj_embed, placeholder_point_traj_embed_ca, placeholder_point_traj_visib

    def streaming_prepare_window_forward(self, 
                              batchsize, len_temporal, window_head, window_tail,
                              pt_tracking_masks, pt_updating_masks,
                              pt_current_traj_features, pt_current_traj_embeds, pt_current_traj_visibs,
                              pt_group_mask=None,
                        ):
        """prepare the required data for the forward pass for each window.
        ...
        Args:
            ...
            window_tail (int): the tail pointer for the window.
            pt_tracking_masks (torch.BoolTensor): [bs, num_point, len_temporal], the mask of the points, whether it should be tracked or not across the whole temporal sequence.
            pt_updating_masks (torch.BoolTensor): [bs, num_point, len_temporal], the mask of the points, whether it should be updated or not across the whole temporal sequence. (the start tracking frame should not be updated)
            pt_current_traj_features (torch.FloatTensor): [bs * len_temporal, num_points, C], the feature for each point across the whole temporal sequence.
            pt_current_traj_embeds (torch.FloatTensor): [bs * len_temporal, num_points, 4], the location for each point across the whole temporal sequence.
            pt_current_traj_visibs (torch.FloatTensor): [bs * len_temporal, num_points], the confidence that the point is visible.
        ...
        """
        if self.streaming_limited_size_of_placeholder == -1:
            pt_current_traj_features = pt_current_traj_features.view(batchsize, len_temporal, *pt_current_traj_features.shape[1:])
        else:
            pt_current_traj_features = pt_current_traj_features.view(batchsize, self.streaming_limited_size_of_placeholder, *pt_current_traj_features.shape[1:])
        pt_current_traj_embeds = pt_current_traj_embeds.view(batchsize, len_temporal, *pt_current_traj_embeds.shape[1:])
        pt_current_traj_visibs = pt_current_traj_visibs.view(batchsize, len_temporal, *pt_current_traj_visibs.shape[1:])
        ...
        if (self.streaming_limited_size_of_placeholder != -1) and window_head >= (self.streaming_limited_size_of_placeholder - 1):
            pt_window_traj_feature = pt_current_traj_features[:, -1:].flatten(0,1)  # if exceed the size of the placeholder, we will store the initial feature to the last frame of placeholder.
        else:
            pt_window_traj_feature = pt_current_traj_features[:, window_head:window_tail+1].flatten(0,1)  # bs*window_size, num_points, C
        pt_window_traj_embeds = pt_current_traj_embeds[:, window_head:window_tail+1].flatten(0,1)  # bs*window_size, num_points, 4
        if self.add_temporal_embedding:
        ...
        return window_temp_attn_mask, window_ltemp_attn_mask, window_self_attn_mask, window_track_mask, window_update_mask, \
            pt_window_traj_feature, pt_window_traj_embeds, pt_window_traj_corr_feature
 
    def streaming_update_query_states(self, 
            window_head, window_tail, len_temporal,\
            window_update_mask, \
            pt_current_traj_embeds, pt_current_traj_embeds_ca, \
            pt_current_traj_visibility, \
            pt_current_traj_features, \
            outputs_coord_list, outputs_coord_list_ca, outputs_class, outputs_hs,\
            padding_to_future=True,
            global_matching_positions=None
        ):
        """Update the trajectory states of the points.
        ...
        Args:
            ...
            window_tail (int): the pointer of the tail of the window.
            window_update_mask (torch.tensor): [bs, num_point, window_size], whether the locations of each points should be updated in the window.
            pt_current_traj_embeds / pt_current_traj_embeds_ca (torch.FloatTensor): [bs, len_temporal, num_points, 4], the location for each point across the whole temporal sequence.
            pt_current_traj_visibility (torch.FloatTensor): [bs, len_temporal, num_points, 3], the visibility for each point across the whole temporal sequence.
            outputs_coords (torch.FloatTensor): [num_dec, bs*window_size, num_points, 4]
            outputs_class (torch.FloatTensor): [num_dec, bs*window_size, num_points, 3]
            global_matching_positions (torch.FloatTensor): [num_points, num_frames, 2], the global matching positions for each point.
            #! pt_current_traj_embeds is unsigmoid, while global_matching_positions is the real position, belonging to (0,1).
        """
        pt_current_traj_embeds = [emb.view(-1, len_temporal, *emb.shape[1:]) for emb in pt_current_traj_embeds]
        pt_current_traj_embeds_ca = [emb.view(-1, len_temporal, *emb.shape[1:]) for emb in pt_current_traj_embeds_ca]
        pt_current_traj_visibility = [emb.view(-1, len_temporal, *emb.shape[1:]) for emb in pt_current_traj_visibility]
        if self.streaming_limited_size_of_placeholder == -1:
            pt_current_traj_features = pt_current_traj_features.view(-1, len_temporal, *pt_current_traj_features.shape[1:])
        else:
            pt_current_traj_features = pt_current_traj_features.view(-1, self.streaming_limited_size_of_placeholder, *pt_current_traj_features.shape[1:])

        window_update_mask_ = window_update_mask.to(outputs_coord_list[-1].dtype).transpose(1,2).unsqueeze(-1)  # [bs, window_size, num_points, 1]
        # 1. updating of trajectory, including location and visibility
        for l_id in range(len(self.transformer.decoder.layers)):
            pt_current_traj_embeds[l_id][:, window_head:window_tail+1]     = pt_current_traj_embeds[l_id].clone()[:, window_head:window_tail+1]     * (1 - window_update_mask_) + outputs_coord_list[l_id].view(-1, self.sliding_window_size, *outputs_coord_list.shape[2:]) * window_update_mask_
            pt_current_traj_visibility[l_id][:, window_head:window_tail+1] = pt_current_traj_visibility[l_id].clone()[:, window_head:window_tail+1] * (1 - window_update_mask_) + outputs_class[l_id].view(-1, self.sliding_window_size, *outputs_class.shape[2:])           * window_update_mask_
        ...
        if self.use_lta_and_no_content_padding:
            if (self.streaming_limited_size_of_placeholder != -1) and window_head >= (self.streaming_limited_size_of_placeholder - 1):
                pt_current_traj_features = torch.cat([
                    pt_current_traj_features[:, 1:self.streaming_limited_size_of_placeholder - 1],  # bs, size-2, C
                    outputs_hs[-1].view(-1, self.sliding_window_size, *outputs_hs[-1].shape[1:])[:, :self.sliding_window_stride],  # bs, 1, C
                    pt_current_traj_features[:, -1:],  # bs, 1, C
                ], dim = 1)
            else:
                pt_current_traj_features[:, window_head:window_head+self.sliding_window_stride] = outputs_hs[-1].view(-1, self.sliding_window_size, *outputs_hs[-1].shape[1:])[:, :self.sliding_window_stride]
        else:
            raise NotImplementedError("In streaming mode, use_lta_and_no_content_padding should be True.")
        
        pt_current_traj_embeds = [emb.flatten(0,1) for emb in pt_current_traj_embeds]
        pt_current_traj_embeds_ca = [emb.flatten(0,1) for emb in pt_current_traj_embeds_ca]
        pt_current_traj_visibility = [emb.flatten(0,1) for emb in pt_current_traj_visibility]
        pt_current_traj_features = pt_current_traj_features.flatten(0,1)
        return pt_current_traj_features, pt_current_traj_embeds, pt_current_traj_embeds_ca, pt_current_traj_visibility
    

class PointTrackingCriterion(SetCriterion):
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, avg_window_loss=True, ignore_outsight=False, ignore_window_loss=False, not_track_invisible=False):
        ...
        self.ignore_window_loss = ignore_window_loss
        self.not_track_invisible = not_track_invisible
    ...
    def forward(self, outputs, targets, return_indices=False):
        ...
        # Full sequence loss from the updating of cross attention
        outputs_full_seq_ca = outputs["full_seq_output_ca"]
        targets_full_seq_ca = self.align_gt_point2box(targets["full_seq_target_ca"])
        matching_indices = [(torch.arange(target["labels"].shape[0]).long(), torch.arange(target["labels"].shape[0]).long()) for bs_id, target in enumerate(targets_full_seq_ca)]
        losses_full_seq_ca_ = super().forward(outputs_full_seq_ca, targets_full_seq_ca, return_indices, matching_indices)
        losses_full_seq_ca  = {}
        for loss_name, loss_value in losses_full_seq_ca_.items():
            losses_full_seq_ca['pt_full_ca_' + loss_name] = loss_value
        losses.update(losses_full_seq_ca)
        # Window loss
        if not self.ignore_window_loss:
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if "mask" in targets[0].keys():
            ...
            num_boxes = src_masks.sum() + 1  # to prevent nan
    ...
    def loss_location(self, outputs, targets, indices, num_boxes):
        ...
        losses = {}
        num_boxes = target_masks.sum() + 1  # to prevent nan
...
def get_weight_dict(args):
    ...
    def get_point_tracking_weight_dict(args):
        ...
        supervise_ca_update = getattr(args, "supervise_ca_update", False)
        weight_ca_update = getattr(args, "weight_ca_update", 1.0)
        ...
        for key, value in weight_dict_.items():
            if "_ce" in key:
                weight_dict['pt_full_ca_' + key] = 0.0
            elif supervise_ca_update:
                weight_dict['pt_full_ca_' + key] = value * weight_ca_update
            else:
                weight_dict['pt_full_ca_' + key] = 0.0
        return weight_dict
    weight_dict_all = {}
...
def get_criterions(args, device, weight_dict):
    ...
    if args.activate_det_seg:
        ...
        criterion = None
    if args.activate_point_tracking:
        not_track_invisible = getattr(args, "not_track_invisible", False)
        ...
        pt_criterion = PointTrackingCriterion(num_classes=args.num_classes, weight_dict=weight_dict, 
                                                     matcher=None, focal_alpha=args.focal_alpha, losses=losses, avg_window_loss=args.avg_window_loss, ignore_outsight=args.ignore_outsight, ignore_window_loss=args.ignore_window_loss, 
                                                     not_track_invisible=not_track_invisible)
...
def build_taptr(args):
    ...
    use_lta_and_no_content_padding = getattr(args, "use_lta_and_no_content_padding", False)
    visibility_aware_in_lta = getattr(args, "visibility_aware_in_lta", False)
    streaming_limited_size_of_placeholder = getattr(args, "streaming_limited_size_of_placeholder", -1)
    ...
    model = TAPTR(
        ...
        use_lta_and_no_content_padding=use_lta_and_no_content_padding,
        visibility_aware_in_lta=visibility_aware_in_lta,

        streaming_limited_size_of_placeholder=streaming_limited_size_of_placeholder,
    )
