...
class CorrBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.S, self.C = len_temp, dimension
        self.padding_mode = padding_mode
        self.num_levels = num_levels
        self.mlvl_spatial_shapes = []
        self.radius = radius
        ...
        for i in range(self.num_levels):
            self.fmap_mappers.append(nn.Conv2d(self.C, self.C, 1))
        self.fmap_mappers = nn.ModuleList(self.fmap_mappers)
        self.fmaps_pyramid = []
        
    def fmap_init(self, memory, mlvl_start_index, mlvl_spatial_shapes):
        """
        memory: torch.tensor, [sum_wh, bs*len_temp, C]
        mlvl_start_index: torch.IntTensor, [num_levels]
        mlvl_spatial_shapes:  torch.IntTensor, [num_levels, 2]
        """
        batch_size = memory.shape[1]//self.S
        num_levels = mlvl_start_index.shape[0]
        mlvl_feat_maps = []
        for start_index, shape in zip(mlvl_start_index, mlvl_spatial_shapes):
            feat_map = memory[start_index: start_index + shape[0] * shape[1]].view(*shape, batch_size, self.S, -1).permute(2, 3, 4, 0, 1)
            mlvl_feat_maps.append(feat_map)
        fmaps = mlvl_feat_maps

        self.fmaps_pyramid = []
        if self.single2multi:
            assert len(fmaps) == 1
            fmaps = fmaps[0]
            mlvl_spatial_shapes = []
            self.fmaps_pyramid.append(self.fmap_mappers[0](fmaps.flatten(0,1)).reshape(batch_size, self.S, *fmaps.shape[2:]))
            for i in range(self.num_levels - 1):
                fmaps_ = fmaps.flatten(0,1)
                fmaps_ = F.avg_pool2d(fmaps_, 2, stride=2)
                fmaps_ = self.fmap_mappers[i+1](fmaps_)
                _, _, H, W = fmaps_.shape
                fmaps = fmaps_.reshape(batch_size, self.S, *fmaps_.shape[1:])
                self.fmaps_pyramid.append(fmaps)
                mlvl_spatial_shapes.append([H, W])
            self.mlvl_spatial_shapes = torch.LongTensor(mlvl_spatial_shapes).to(fmaps.device)
            return 
        for i in range(num_levels):
            fmaps_ = fmaps[i].flatten(0,1)
            fmaps_ = self.fmap_mappers[i](fmaps_)
            fmaps_ = fmaps_.reshape(batch_size, self.S, *fmaps_.shape[1:])
            self.fmaps_pyramid.append(fmaps_)
        self.mlvl_spatial_shapes = mlvl_spatial_shapes

    def sample(self, coords_, scale_wh=None):
        """
        coords_: torch.tensor, [N, bs*len_temp, 2]
        """
        batch_size = coords_.shape[1]//self.S
        coords = coords_.view(-1, batch_size, self.S, 2).permute(1, 2, 0, 3)
        coords = coords * (self.mlvl_spatial_shapes[0][None, None, None, :] - 1)
        r = self.radius
        ...
        out = out.flatten(0,1).permute(1, 0, 2)  # [bs*len_temp, N, C]  -->  [N, bs*len_temp, C]
        # out = out.permute(0, 2, 1, 3).contiguous().view(B * N, S, -1).float()
        return out
    
    def corr(self, targets_):
        """
        targets: torch.tensor, [N, bs*len_temp, C]
        """
        batch_size = targets_.shape[1]//self.S
        targets = targets_.permute(1,0,2).view(batch_size, self.S, -1, self.C)
        ...
        self.corrs_pyramid = []  # list, len=num_levels, each element is [B, S, N, H, W]
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
                 ...
                 num_global_decoder_layers=0,
                 global_corr_once=False,
                 fix_decoder_unsync=False,
                 ignore_wh_inca=False,
                 position_aware_mem_mode=False,
        ...
        if deformable_decoder:
            decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          ...
                                                          position_aware_mem_mode=position_aware_mem_mode,
                                                          use_checkpoint=use_checkpoint,
                                                        )
        ...
        self.global_corr_once = global_corr_once
        if num_global_decoder_layers > 0:  # desperate global layers.
            raise NotImplementedError
            global_decoder_layer = GlobalTransformerDecoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, dec_n_points, use_deformable_box_attn=use_deformable_box_attn, box_attn_type=box_attn_type,
                                                          key_aware_type=key_aware_type,
                                                          decoder_sa_type=decoder_sa_type,
                                                          module_seq=module_seq,
                                                          global_corr_once=global_corr_once)
        else:
        ...
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                        ...
                                        global_decoder_layer=global_decoder_layer, num_global_decoder_layers=num_global_decoder_layers, global_corr_once=global_corr_once,
                                        fix_decoder_unsync=fix_decoder_unsync,
                                        use_checkpoint=use_checkpoint,
                                        )
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
                    ...
                    use_detached_boxes_dec_out=False,
                    global_decoder_layer=None, num_global_decoder_layers=0, global_corr_once=False,
                    fix_decoder_unsync=False,
                    use_checkpoint=False,
                    ):
        super().__init__()
        ...
        self.num_global_decoder_layers = num_global_decoder_layers
        self.global_corr_once = global_corr_once
        if num_global_decoder_layers > 0:
            self.global_layers = _get_clones(global_decoder_layer, num_global_decoder_layers, layer_share=False)
        else:
        ...
        # corr
        self.add_corr_block = False
        self.corr_block = None
        self.corr_once = False
        self.corr_query_proj = None
        # for second corr
        self.corr_block_2 = None
        # update query feature between layers.
        self.update_query_mode = "normal"
        self.query_updater = None
        self.fix_decoder_unsync = fix_decoder_unsync
        if self.fix_decoder_unsync:
            self.norm = None
    ...
    def forward(self, tgt, tgt_corr, memory,
        ...
        if self.add_corr_block:
            self.corr_block.fmap_init(memory, level_start_index, spatial_shapes)
            if self.corr_once:
                self.corr_block.corr(self.corr_query_proj(tgt_corr))

        for layer_id, layer in enumerate(self.global_layers + self.layers):
            # if use global decoder, to not effect the original local decoder, before doing local decoder:
            # if corr_once: we use the original feature to reinitialize the correlation map.
            # but when corr_block also share the corr map, we do not need to reinitialize them again.
            if (self.num_global_decoder_layers > 0) and (layer_id == self.num_global_decoder_layers) and self.corr_once and (not self.global_corr_once):
                self.corr_block.corr(self.corr_query_proj(output_init))
            ...
            if not dropflag:
                input_to_layer = None
                if self.update_query_mode == "normal":
                    input_to_layer = output
                elif self.update_query_mode == "init":
                    input_to_layer = output_init
                elif self.update_query_mode == "residual":
                    input_to_layer = output + output_init
                elif self.update_query_mode == "mlp_delta":
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        if "ca" in module_seq:
            ...
            self.position_aware_mem_mode = position_aware_mem_mode
            if self.position_aware_mem_mode == "mlp_fuse":
                self.memory_pos_fuser = nn.Sequential(*[
                    nn.Linear(2 * d_model, d_model + d_model // 2),
                    nn.ReLU(),
        ...
        if 'ha' in module_seq:
            self.history_attention = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
            self.dropout_ha = nn.Dropout(dropout)
            self.norm_ha = nn.LayerNorm(d_model)
        if 'hmva' in module_seq:
            self.history_mean_var_attention = nn.Sequential(*[
                nn.Linear(d_model * 3, d_model),
                nn.ReLU(),
                nn.Linear(d_model, d_model),
            ])
            self.norm_hmva = nn.LayerNorm(d_model)
        if "dca" in module_seq:
            self.dense_cross_attn = nn.MultiheadAttention(d_model, n_heads, dropout=dropout)
            self.dense_levels = [1,2,3]
            self.dropout_dca = nn.Dropout(dropout)
            self.norm_dca = nn.LayerNorm(d_model)
    ...
    def forward_ca(self,
        ...
        if self.position_aware_mem_mode == "mlp_fuse":
            memory = self.memory_pos_fuser(torch.cat([memory, memory_pos], dim=-1))
        elif self.position_aware_mem_mode == "add":
            memory = memory + memory_pos
        elif self.position_aware_mem_mode == "no":
            pass
        else:
            raise NotImplementedError 
    ...
    def forward(self,
        ...
        if self.add_corr_block:
            ...
            fcorrs = corr_block.sample(tgt_reference_points[:,:,0][..., :2], tgt_reference_points[:,:,0][..., 2:])
    ...
    def get_flow_embedding(self, xy) -> torch.Tensor:
        """
        ...
        B, N, D = xy.shape
        assert D == 2
        C = self.dim_flow_embed
        cat_coords = self.cat_coord_to_fe
        ...
        # ).reshape(1, 1, int(C / 2))
        div_term = torch.arange(0, C, 2, dtype=torch.float32, device=x.device) / C
        div_term = 2*math.pi/(5 ** div_term)
