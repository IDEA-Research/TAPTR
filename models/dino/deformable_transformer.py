...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        assert (sorted(module_seq) == ['ca', 'ffn', 'sa', 'tsa']) or (sorted(module_seq) == ['ca', 'ffn', 'sa']) or (sorted(module_seq) == ['ca', 'dca', 'ffn', 'sa', 'tsa']) or (sorted(module_seq) == ['dca', 'ffn', 'sa', 'tsa']) \
                or (sorted(module_seq) == ['ca', 'ffn', 'ha', 'sa', 'tsa']) or (sorted(module_seq) == ['ca', 'ffn', 'hmva', 'sa', 'tsa']) or (sorted(module_seq) == ['ffn', 'sa', 'tsa']) or (sorted(module_seq) == ['ffn', 'ldca', 'sa', 'tsa']) \
                or (sorted(module_seq) == ['ffn', 'mdca', 'sa', 'tsa']) or (sorted(module_seq) == ['aw_ca', 'ffn', 'sa', 'tsa']) \
                or (sorted(module_seq) == ['bili', 'ca', 'ffn', 'sa', 'tsa']) 
        # cross attention
        if ("ca" in module_seq) or ("aw_ca" in module_seq):
            if use_keyaware_ca:
                self.cross_attn = MSDeformAttn_KeyAware(d_model, n_levels, n_heads, n_points, ignore_wh_inca=ignore_wh_inca, deformable_use_checkpoint=use_checkpoint)
            else:
    ...
    def forward_ca(self,
        ...
        if self.update_pos_in_ca:
            tgt2, attention_weights_before_sfmx, sampling_offsets = self.cross_attn(
                               ...
                               input_pe_flatten=memory_pos.transpose(0, 1),)
