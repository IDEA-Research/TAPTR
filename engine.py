...
def train_one_epoch(model: torch.nn.Module, criterions: torch.nn.Module,
    ...
    # path_ckpt = "logs/baseline_NoSizeL_B1L24CR512_UnrollTrain_TE_IgnoreOutsight145Ep_AugPhotoPanFlipRestart_FixRatio_FewerEpoch_CoTrackerSampler_FixLossDeno/checkpoint.pth"
    # updated_param = torch.load(path_ckpt, map_location="cpu")["model"]
    # new_param = torch.load("debug.pth", map_location="cpu")
    # for name, param in updated_param.items():
    #     if "backbone" in name:
    #         print(name, (param - new_param[name]).abs().sum())
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            # from ipdb import set_trace; set_trace()
            # # debug_visualize_results(samples, outputs["full_seq_output"], targets["full_seq_target"])
            # debug_visualize_all_results(samples, outputs, targets, sliding_window_stride=model.sliding_window_stride, sliding_window_size=model.sliding_window_size)
            loss_dict = {}
        ...
        #     loss_dict_[name.replace("pt_", "ptc_")] = value
        # loss_dict = loss_dict_
        # weight_dict_ = {}
        # for name, value in weight_dict.items():
        #     weight_dict_[name.replace("pt_", "ptc_")] = value
        # weight_dict = weight_dict_
        # #! debug only
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    ...
    model.eval()
    # TODO: fix this
    criterion = criterion["ptc_criterion"]
    criterion.eval()
